        ...
        {watchlist.map((ticker) => {
          ...
          return (
            <button
              ...
              className="shrink-0 flex items-center gap-4 px-5 py-3.5 bg-white/[0.03] border border-border rounded-lg hover:border-border-hover hover:bg-white/[0.06] transition-all group"
            >
              <span className="text-base font-semibold text-accent font-[family-name:var(--font-mono)]">
                {ticker}
              </span>
              {data?.current_price && (
              ...
              {data?.price_change_pct != null && (
                <span className={`text-sm font-medium font-[family-name:var(--font-mono)] ${data.price_change_pct >= 0 ? 'text-accent' : 'text-danger'}`}>
